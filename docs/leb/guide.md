...
JPL. The interpolated apsides (`INTP_APOG`, `INTP_PERG`) are served by the
packaged lunar-apse model rather than by a stored channel, so in `leb` mode
they are bounded by that model's fitted window — JD 2286820.5 to 2689310.5
(apogee) and to 2689316.5 (perigee), 1549-01-01 to late December 2650 in TT,
read from the residual-grid metadata in `lunar_apse_model` — whatever groups
are installed, and by the `apogee` group's stored interval as well when one is
attached. Curated minor bodies without meaningful LEB coverage may use an
existing deterministic local model, and tracing reports its real source as
`Keplerian` or `Analytical`. Stored LEB trajectories generated by a reviewed
numerical model remain `source=LEB` and declare
`precision_class=numerical-model`.
...
per-body ranges. For the interpolated apsides the error may instead carry
the fitted window of their model (1549--2650, no `ephemeris_file`), which
applies whether or not an `apogee` group is installed (section 2).
