...
The wheel contains `base_core.leb2`, covering 14 core bodies. Other published
LEB2 groups and DE/planet-center files are optional downloads. The LEB format
and generators support an `exotics` companion group, but no exotics artifact is
currently listed in the release download manifest; users must generate or
provide that companion themselves. Missing bodies continue through the normal
fallback chain.
