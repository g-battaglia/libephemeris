...
| `finite-source-strict-proposer` | Private finite-policy Decimal proposal of strict per-target certificates | Reviewed [project method](finite-source-strict-proposer.md); every decisive candidate passes the independent exact verifier; no runtime route or whole-call bound |
| `finite-source-algebraic-contact-verifier` | Private exact algebraic validation of one per-target equality contact certificate | Project-authored [derivation](finite-source-algebraic-contact.md), reviewed disconnected specification; no generator, runtime cap, astronomical source or public claim |
| `finite-source-contact-algebra` | Private exact polynomial factors, real-root isolation and algebraic sign decisions for the contact producer | Project-authored [derivation](finite-source-contact-generator.md) and exact python-flint arithmetic; no verifier internals or public claim |
| `finite-source-contact-extension` | Private exact quadratic extension for positive-singular contact points | Project-authored [derivation](finite-source-contact-generator.md) and rational multiplication matrices; no astronomical source or public claim |
| `finite-source-contact-generator` | Private fixed-state enumeration of contact candidates for one target | Project-authored [derivation](finite-source-contact-generator.md), reviewed disconnected contract and independent algebraic verifier; no runtime cap or public claim |
| `finite-source-contact-request-wire` | Private in-memory canonical request bytes for exact fixed-state geometry | Project-authored [non-normative design candidate](finite-source-contact-wire.md) and independently specified byte fixture; no transport bound, worker, response or public claim |
...
| `private-sun-moon-state-reducer` | Disconnected owned-source Sun/Moon state diagnostic | Reviewed [bounded contract](private-sun-moon-state-reducer.md), exact selected-child and branch observations; incomplete transitive inventory, so no source receipt |
