...
Code identified as unused by any live calculation path. Each entry records
whether it was retained or removed, why it was dead, and what replaced it.
...
- True-node and osculating-apsis calculations use JPL state-vector geometry.
  Public mean points delegate to the ERFA/IERS implementation in
...
## Lunar apse model and retired generators

- `lunar_apse_model.py` is the interpolated-apsides model, generated from the
  JPL DE440 apsis passages by the committed
  `scripts/generate_lunar_apse_model.py` (a fully reproducible derivation).
  Its exact SHA-256 is enforced; regeneration goes through that script only.
  The retired binary models and their generation commands remain absent. The
  alternative JPL-state smoother is retained for research but is not the
  public INTP path.
...
- The whole table and its table-only tests were removed in the 2026-07-13
  provenance cleanup. It had no summation consumer; the live true node is
  geometric.
...
### libephemeris/hypothetical.py (retired Planet X duplicate paths)

- The duplicate Lowell/Pickering raw helpers, dataclasses, and divergent
  element constants were removed in the 2026-07-13 clean-room consolidation.
  Supported IDs 40–54 now have one canonical, hash-gated CSV/runtime mapping.
