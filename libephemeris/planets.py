...
def _calc_pheno_leb(tjd_ut: float, ipl: int, iflag: int) -> Tuple[float, ...]:
    ...
    # Preserve correction flags that affect the position pipeline
    base_flags = SEFLG_SPEED | (iflag & (SEFLG_TRUEPOS | SEFLG_NOABERR | SEFLG_NOGDEFL))
    ...
    # ------------------------------------------------------------------
    if ipl == SE_MOON:
        # Moon: use 3D vector dot product for exact phase angle, matching
        # _calc_pheno(). The scalar law-of-cosines loses precision for the
        # nearly degenerate Sun-Moon-Earth triangle.
        try:
            from .constants import SEFLG_XYZ, SEFLG_EQUATORIAL, SEFLG_J2000

            moon_xyz, _ = _leb_calc(tjd_ut, SE_MOON, SEFLG_XYZ | SEFLG_EQUATORIAL | SEFLG_J2000)
            sun_xyz, _ = _leb_calc(tjd_ut, SE_SUN, SEFLG_XYZ | SEFLG_EQUATORIAL | SEFLG_J2000)
            # Moon→Sun and Moon→Earth vectors
            ms = (sun_xyz[0] - moon_xyz[0], sun_xyz[1] - moon_xyz[1], sun_xyz[2] - moon_xyz[2])
            me = (-moon_xyz[0], -moon_xyz[1], -moon_xyz[2])
            dot = ms[0] * me[0] + ms[1] * me[1] + ms[2] * me[2]
            mag_ms = math.sqrt(ms[0] ** 2 + ms[1] ** 2 + ms[2] ** 2)
            mag_me = math.sqrt(me[0] ** 2 + me[1] ** 2 + me[2] ** 2)
            if mag_ms > 0 and mag_me > 0:
                cos_pa = max(-1.0, min(1.0, dot / (mag_ms * mag_me)))
                phase_angle = math.degrees(math.acos(cos_pa))
            else:
                phase_angle = 180.0 - elongation
        except (KeyError, ValueError):
            phase_angle = 180.0 - elongation
    elif geo_dist > 0 and helio_dist > 0:
