...
@generate_group.command(
    "apse-corrections",
    short_help="Regenerate interpolated apogee/perigee residual tables.",
)
def apse_corrections() -> None:
    """Regenerate libephemeris/lunar_apse_corrections.py (the live table).

    Recomputes the residuals (reference - mean - trig series) for the
    interpolated apogee and perigee on the full correction grid.  Run
    this after updating the ELP-style perturbation coefficients via
    'leph calibrate perigee'.  Requires pyswisseph as the oracle.
    """
    _python(["scripts/generate_lunar_apse_corrections.py", "--write"])
...
def lunar_corrections() -> None:
    ...
    CLEAN.md); the live interpolated-apse residuals are regenerated by
    'leph generate apse-corrections' instead.  Kept for reference.
    """
