...
def _calculate_local_eclipse_phases(
    ...
    if reader is not None:
        # Verify LEB covers this date range before defining closures
        try:
            from .fast_calc import _topo_ecliptic
            from .time_utils import swe_deltat
            _topo_ecliptic(reader, tjdut + swe_deltat(tjdut), tjdut, SE_SUN, (lon, lat, altitude))
        except (KeyError, ValueError):
            reader = None  # Fall through to Skyfield
...
def sol_eclipse_when_loc(
    ...
    for _ in range(MAX_ECLIPSES):
        ...
        if jd_local_max > 0 and magnitude > 0:
            ...
            # Get Moon/Sun distances at local maximum (topocentric for accuracy)
            if reader is not None:
                from .time_utils import swe_deltat as _sd_loc
                _jd_tt_loc = jd_local_max + _sd_loc(jd_local_max)
                _sun_loc = _topo_ecliptic(reader, _jd_tt_loc, jd_local_max, SE_SUN, geopos)
                _moon_loc = _topo_ecliptic(reader, _jd_tt_loc, jd_local_max, SE_MOON, geopos)
                sun_dist_au = _sun_loc[2]
                moon_dist_au = _moon_loc[2]
            else:
                sun_pos, _ = swe_calc_ut(jd_local_max, SE_SUN, SEFLG_SPEED)
...
def swe_sol_eclipse_how_details(
    ...
    if reader is not None:
        ...
        def _calc_position_angle(jd: float) -> float:
            ...
            sun_eq, _ = swe_calc_ut(jd, SE_SUN, SEFLG_EQUATORIAL | SEFLG_SPEED)
    ...
    if _use_leb_rt:
        try:
            if not is_fixed_star:
                swe_calc_ut(jd_start, body, SEFLG_SPEED)
        except (KeyError, ValueError):
            _use_leb_rt = False
        ...
        try:
            ...
            if visible:
                ...
                if _reader is not None:
                    ...
                    occ_pos, _ = swe_calc_ut(jd_max, occulting_planet, SEFLG_SPEED)
                    if occulted_planet == 0:
                        from .fixed_stars import swe_fixstar_ut
                        tgt_pos, _, _ = swe_fixstar_ut(star_name, jd_max, SEFLG_SPEED)
                        target_radius = 0.0001
                    else:
                        tgt_pos, _ = swe_calc_ut(jd_max, occulted_planet, SEFLG_SPEED)
