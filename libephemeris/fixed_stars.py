...
class FixedStarInfo:
    """Public fixed-star catalog metadata."""
...
def list_fixed_stars() -> Tuple[FixedStarInfo, ...]:
    """Return the native libephemeris fixed-star catalog.

    The catalog is intentionally independent from Swiss Ephemeris data files.
    Consumers that need backend-neutral fixed-star discovery can use this list
    with ``batch_fixstars_ut()`` instead of reading external catalog files.
    """
    return tuple(
        FixedStarInfo(
            name=entry.name,
...
def _calc_star_position_from_observer(
    ...
    """Calculate one star from a precomputed Earth-at-time position."""
...
def swe_batch_fixstars_ut(
    ...
    *,
    ...
    This is a libephemeris extension, not a pyswisseph API. It preserves the
    input order and reuses the shared Earth position for all stars at the same
    instant. When ``SEFLG_SPEED`` is requested, the three finite-difference
    instants are shared across the batch as well.

    Args:
        stars: Star names resolvable by ``swe_fixstar_ut``.
        tjdut: Julian Day in Universal Time (UT1).
        flags: Calculation flags.
        skip_errors: If True, unresolved or failed stars return ``None`` in
            their original slot. If False, the first error is raised.

    Returns:
        Tuple with one result per input star. Each non-None item has the same
        shape as ``swe_fixstar_ut``: ``(position_tuple, canonical_name, flags)``.
    """
    flags = _preprocess_flags(flags)
