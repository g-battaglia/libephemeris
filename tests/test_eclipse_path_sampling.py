...
by making the sampler's per-step dependency fail if it is ever reached. A
...
# A short, well-formed window. Only the step is under test.
_JD_START = 2451545.0
_JD_END = 2451545.1
...
@pytest.mark.parametrize(
    ("label", "sampler"), _SAMPLERS, ids=lambda v: v if isinstance(v, str) else ""
)
@pytest.mark.parametrize("step_minutes", [0.0, -0.0, -1.0, -0.5, -1e6])
def test_a_non_positive_step_is_refused_before_sampling(label, sampler, step_minutes):
...
def test_a_step_too_small_to_advance_the_date_is_refused(label, sampler, step_minutes):
    """Positive, but inert: `jd + step` is the same float as `jd`."""
    assert _JD_START + step_minutes / (24.0 * 60.0) == _JD_START
    with _no_sampling_allowed():
        with pytest.raises(InputValidationError):
            sampler(_JD_START, _JD_END, step_minutes=step_minutes)
...
def test_a_non_finite_step_is_refused(label, sampler, step_minutes):
    with _no_sampling_allowed():
        with pytest.raises(InputValidationError):
            sampler(_JD_START, _JD_END, step_minutes=step_minutes)
...
def test_a_non_finite_window_is_refused(label, sampler, jd_start, jd_end):
    """Two infinite bounds keep the loop condition true with no progress."""
    with _no_sampling_allowed():
        with pytest.raises(InputValidationError):
            sampler(jd_start, jd_end, step_minutes=1.0)
...
def test_a_step_that_advances_the_date_is_accepted(label, sampler, step_minutes):
    ...
    jd_end = _JD_START + 3.0 * step_days

    times, latitudes, longitudes = sampler(_JD_START, jd_end, step_minutes=step_minutes)
...
def test_an_inverted_window_returns_nothing_rather_than_raising(label, sampler):
    """jd_end before jd_start is an empty window, not an invalid one."""
    assert sampler(_JD_END, _JD_START, step_minutes=1.0) == ((), (), ())
...
def test_the_default_step_is_still_valid(label, sampler):
    """The documented default must not be rejected by its own guard."""
    times, _latitudes, _longitudes = sampler(_JD_START, _JD_START + 0.01)
    assert isinstance(times, tuple)
...
def test_the_message_names_the_function_and_the_offending_value():
    with pytest.raises(InputValidationError) as excinfo:
        calc_eclipse_central_line(_JD_START, _JD_END, step_minutes=0.0)
...
def test_the_error_stays_inside_the_documented_hierarchy():
    ...
    with pytest.raises(Error):
        calc_eclipse_central_line(_JD_START, _JD_END, step_minutes=-1.0)
...
def test_the_advancement_rule_follows_the_date_magnitude():
    ...
    assert _JD_START + step_days == _JD_START  # inert at a Julian date

    with _no_sampling_allowed():
        with pytest.raises(InputValidationError):
            calc_eclipse_central_line(_JD_START, _JD_END, step_minutes=step_minutes)
