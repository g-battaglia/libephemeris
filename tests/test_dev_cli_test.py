...
from libephemeris.dev_cli import cmd_completion, cmd_leb, cmd_leb2, cmd_test
...
from scripts import generate_leb2, test_leb2_precision
...
def test_leb_help_reports_eligible_inventories_and_preserves_examples() -> None:
    ...
    assert result.exit_code == 0
    assert "~375 MB, 62 registered bodies" in result.output
    assert "~1.23 GB, 62 registered bodies" in result.output
    assert "~7.64 GB, 54 bodies (8 NEAs excluded)" in result.output
    assert (
...
def test_user_cli_describes_canonical_and_published_leb2_groups() -> None:
    """User-facing help distinguishes five groups from four release assets."""
    runner = CliRunner()
    config_result = runner.invoke(user_cli, ["config"])
    download_result = runner.invoke(user_cli, ["download", "leb2-base", "--help"])

    assert config_result.exit_code == 0
    assert "Published LEB1 download assets" in config_result.output
    assert "31 bodies" in config_result.output
    assert "5 canonical groups" in config_result.output
    assert "{tier}_exotics.leb2" in config_result.output
    assert download_result.exit_code == 0
    assert "4 currently published groups" in download_result.output
    assert "canonical exotics group" in download_result.output


def test_init_wizard_does_not_require_unpublished_exotics() -> None:
    """Wizard requirements match the four downloadable release companions."""
    ...
    assert sum(status == "required" for status in leb2.values()) == 4


def test_init_wizard_uses_current_canonical_exotics_sizes() -> None:
    """Optional estimates follow the canonical generated rc8 companions."""
    assert init_wizard._LEB2_SIZES["base"]["exotics"] == 59.0
    assert init_wizard._LEB2_SIZES["medium"]["exotics"] == 194.8
    assert init_wizard._LEB2_SIZES["extended"]["exotics"] == 283.9
