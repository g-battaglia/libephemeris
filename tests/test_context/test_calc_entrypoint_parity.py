...
@pytest.mark.parametrize("body", [le.MEAN_NODE, le.MEAN_APOG])
@pytest.mark.parametrize("representation", [0, le.FLG_XYZ, le.FLG_RADIANS])
def test_skyfield_mean_point_speed3_is_centered_reported_derivative(
    monkeypatch, use_context, body, representation
):
    """Module and context fallbacks differentiate the final TT position."""
    monkeypatch.setattr(state, "get_leb_reader", lambda: None)
    monkeypatch.setattr(state, "get_horizons_client", lambda: None)
    api = le.EphemerisContext() if use_context else le
    h = 0.0001
    position_flags = le.FLG_SWIEPH | representation
    current = api.calc(JD, body, position_flags | le.FLG_SPEED3)[0]
    previous = api.calc(JD - h, body, position_flags)[0]
    following = api.calc(JD + h, body, position_flags)[0]

    if representation & le.FLG_XYZ:
        expected = tuple(
            (following[index] - previous[index]) / (2.0 * h) for index in range(3)
        )
    else:
        cycle = 2.0 * math.pi if representation & le.FLG_RADIANS else 360.0
        longitude_delta = (
            following[0] - previous[0] + cycle / 2.0
        ) % cycle - cycle / 2.0
        expected = (
            longitude_delta / (2.0 * h),
            (following[1] - previous[1]) / (2.0 * h),
            (following[2] - previous[2]) / (2.0 * h),
        )

    assert current[3:] == pytest.approx(expected, abs=1e-10)
