...
def test_sdist_runtime_data_is_exactly_allowlisted() -> None:
    """A newly introduced binary/data file cannot enter an sdist by glob."""
    manifest = (_PROJECT_ROOT / "MANIFEST.in").read_text()
...
def test_bundled_core_matches_reviewed_manifest_hash() -> None:
    """The wheel must contain exactly the independently regenerated core."""
    from libephemeris.download import DATA_FILES

    core = _PROJECT_ROOT / "libephemeris/data/leb2/base_core.leb2"
