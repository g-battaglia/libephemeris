...
# ---------------------------------------------------------------------------
# the module's heliacal helpers
# ---------------------------------------------------------------------------
class TestHeliacal:
    def test_event_type(self):
        with pytest.raises(
            Error,
            match=r"event_type 9 is not a heliacal event type: use HELIACAL_RISING, "
            r"HELIACAL_SETTING, EVENING_FIRST or MORNING_LAST\.",
        ):
            eclipse.heliacal_ut(JD, 41.9, 12.5, body=3, event_type=9)

    def test_the_sun(self):
        with pytest.raises(
            Error, match=r"The Sun is not a valid object for heliacal calculations\."
        ):
            eclipse.heliacal_ut(JD, 41.9, 12.5, body=libephemeris.SUN)

    def test_the_moon(self):
        with pytest.raises(
            Error, match=r"The Moon is not a valid object for heliacal calculations\."
        ):
            eclipse.heliacal_ut(JD, 41.9, 12.5, body=libephemeris.MOON)

    def test_body_outside_the_planet_table(self):
        with pytest.raises(
            Error, match=r"Body 99 is not a body heliacal calculations can place\."
        ):
            eclipse.heliacal_ut(JD, 41.9, 12.5, body=99)
...
class TestVisLimitMag:
    ...
    def test_empty_name(self):
        with pytest.raises(
            Error,
            match=r"objname must name a planet or a fixed star; it cannot be empty\.",
        ):
            eclipse.vis_limit_mag(JD, ROME, self.ATMO, self.OBSERVER, "")

    def test_star_that_cannot_be_resolved(self, monkeypatch):
        from libephemeris import fixed_stars

        def _broken(*_args, **_kwargs):
            raise IndexError("catalogue row missing")

        monkeypatch.setattr(fixed_stars, "fixstar2_ut", _broken)
        with pytest.raises(
            Error,
            match=r"The star name 'nosuchstar' could not be resolved: catalogue "
            r"row missing",
        ) as excinfo:
            eclipse.vis_limit_mag(JD, ROME, self.ATMO, self.OBSERVER, "NoSuchStar")
        assert isinstance(excinfo.value.__cause__, IndexError)

    def test_body_outside_the_planet_table(self):
        with pytest.raises(
            Error,
            match=r"Body 99 is not a body the visibility limit can be computed for\.",
        ):
            eclipse.vis_limit_mag(JD, ROME, self.ATMO, self.OBSERVER, "99")
